  ...
  <div class="jumbotron">
    <article <?php post_class( 'article-content' ); ?>>
      <div class="article-info">
        <?php if(has_category() ): ?>
        <span class="cat-data">
          <!-- カテゴaリー表示 --> <?php $this_categories = get_the_category();if ( $this_categories ) {$this_category_color = get_field( 'color', 'category_' . $this_categories[0]->term_id );$this_category_name  = $this_categories[0]->name; echo '<span class="entry-label" style="' . esc_attr( 'background:' . $this_category_color ) . ';">' . esc_html( $this_category_name ) . '</span>';}?>
        </span>
